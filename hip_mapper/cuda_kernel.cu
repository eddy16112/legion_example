                     ...
                     Context ctx, Runtime *runtime)
{
  ...
  hipError_t err = hipMemcpy_H(cpu_buff, ptr_x, sizeof(int)*rect.volume(), hipMemcpyDeviceToHost);
  assert(err == hipSuccess);
  for(int i = 0; i < rect.volume(); i++) {
    cpu_buff[i] += 1;
  }
  err = hipMemcpy_H(ptr_x, cpu_buff, sizeof(int)*rect.volume(), hipMemcpyHostToDevice);
  assert(err == hipSuccess);
  
  err = hipMemcpy_H(cpu_buff, ptr_y, sizeof(int)*rect.volume(), hipMemcpyDeviceToHost);
  assert(err == hipSuccess);
  for(int i = 0; i < rect.volume(); i++) {
    cpu_buff[i] += 2;
  }
  err = hipMemcpy_H(ptr_y, cpu_buff, sizeof(double)*rect.volume(), hipMemcpyHostToDevice);
  free(cpu_buff);
#else
